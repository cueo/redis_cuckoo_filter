...
def insert(keys):
	...
	for key in keys:
		...
		if hash_value == 0:
			filter_name = 'b'
			print 'Redirected to node at 127.0.0.1:30001'
		elif hash_value == 1:
			filter_name = 'c'
			print 'Redirected to node at 127.0.0.1:30002'
		else:
			filter_name = 'a'
			print 'Redirected to node at 127.0.0.1:30003'
		cmd = ['cuckooinsertelement', filter_name, key]
		try:
			result = rc.execute_command(*cmd)
			if result[0] == 1:
				print 'Success'
			else:
				print 'Failure'
...
def reset():
	'''
	# This functionality is basically to start afresh.
	# It is simply the deletion of all the filters followed by creation.
	'''
	delete()
	create()
